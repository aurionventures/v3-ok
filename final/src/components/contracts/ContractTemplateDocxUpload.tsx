...
export default function ContractTemplateDocxUpload({
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isDocx && !isHtml) {
      toast.error("Selecione um arquivo .docx ou .html");
      event.target.value = "";
      return;
    }
    ...
    try {
      ...
      toast.success(isHtml ? "HTML importado com sucesso" : "DOCX importado com sucesso");
    } catch (err) {
  ...
  return (
    <>
      ...
      <Button variant="outline" onClick={handlePick} disabled={isImporting}>
        ...
        {isImporting ? "Importando..." : "Importar DOCX/HTML"}
      </Button>
    </>
  );
}
