...
export function MemberApprovalModal({ open, onClose, ata, onApprovalComplete }: MemberApprovalModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    if ((action === "revision" || action === "reject") && !comments.trim()) {
      toast.error("Informe o motivo da revisão/rejeição");
      return;
    }
          ...
          <div className="space-y-5">
                ...
                <div className={`flex items-center gap-3 p-3 border rounded-lg cursor-pointer transition-colors ${action === "revision" ? "border-amber-500 bg-amber-50" : "hover:bg-muted/50"}`}>
                  ...
                  <Label htmlFor="revision" className="flex items-center gap-2 cursor-pointer flex-1">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Há pontos que precisam de correção</p>
                    </div>
                  </Label>
                </div>
            ...
            <div className="space-y-2">
              <Label htmlFor="comments">
                Comentários {(action === "revision" || action === "reject") && <span className="text-red-500">*</span>}
              </Label>
              <Textarea
                id="comments"
                placeholder={
                  action === "approve" 
                    ...
                    : action === "revision"
                    ? "Descreva os pontos que precisam de revisão..."
