...
import { fetchPerfilSuperAdminByUserId } from "@/services/empresas";
...
const AdminAlterarSenha = () => {
  ...
  useEffect(() => {
    const check = async () => {
      ...
      if (!session?.user?.id) {
        navigate("/login", { replace: true, state: { userType: "admin" } });
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (dbError) {
        console.error("[AdminAlterarSenha] update senha_alterada:", dbError);
        toast({
        ...
        toast({
          ...
          description: "Agora você pode acessar o painel administrativo com sua nova senha.",
        });
      }
    ...
    <div className="flex h-screen bg-gray-50">
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="border-b bg-background px-6 py-4">
          <h1 className="text-xl font-semibold text-gray-900">Legacy OS</h1>
        </header>
        <div className="flex-1 flex items-center justify-center p-6 overflow-y-auto">
          <Card className="w-full max-w-md">
            <CardHeader>
              ...
              <CardDescription>
                Esta é sua primeira vez acessando como Super Admin. Por segurança, defina uma nova senha.
              </CardDescription>
            </CardHeader>
            <CardContent>
...
export default AdminAlterarSenha;
