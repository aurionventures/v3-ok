...
import { AlertTriangle, Shield, DollarSign, Settings, BarChart3, Plus, Eye, Edit, Target, TrendingUp, Clock, CheckCircle, AlertCircle } from "lucide-react";
...
const mitigationPlans = [{
  ...
  actions: [{
    id: 1,
    description: "Implementar sistema de intelligence competitiva",
    deadline: "2024-10-31",
    status: "in_progress",
    progress: 60
  }, {
    id: 2,
    description: "Desenvolver plano de inovação disruptiva",
    deadline: "2024-12-15",
    status: "planned",
    progress: 0
  }]
  ...
  actions: [{
    id: 3,
    description: "Renegociar linhas de crédito",
    deadline: "2024-09-30",
    status: "completed",
    progress: 100
  }, {
    id: 4,
    description: "Implementar cash flow forecasting avançado",
    deadline: "2024-11-15",
    status: "in_progress",
    progress: 40
  }]
...
const GovernanceRiskManagement = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "mitigated":
        return <Badge className="bg-green-100 text-green-800">Mitigado</Badge>;
      default:
...
${comments ? `Considerando seus comentários: "${comments}", sugiro também focar em aspectos específicos mencionados.` : ''}`;
  ...
  const filteredRisks = governanceRisks.filter(risk => {
    const matchesSearch = risk.title.toLowerCase().includes(searchTerm.toLowerCase()) || risk.description.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return <div className="h-screen flex w-full">
            ...
            <TabsTrigger value="intelligence">Inteligência de Mercado</TabsTrigger>
                ...
                <TabsContent value="matrix" className="space-y-6">
                  <div className="flex gap-4 mb-6">
                    <Input placeholder="Buscar riscos..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="max-w-sm" />
                    <Select value={categoryFilter} onValueChange={setCategoryFilter}>
                      ...
                      <SelectContent>
                        ...
                        {RISK_CATEGORIES.map(cat => <SelectItem key={cat.id} value={cat.id}>{cat.name}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                          ...
                          <CardContent className="p-4">
                            ...
                            backgroundColor: `${category.color}20`
                              ...
                              <div>
                                ...
                                {criticalCount > 0 && <Badge variant="destructive" className="mt-1">{criticalCount} críticos</Badge>}
                              </div>
                            </div>
                          </CardContent>
                        </Card>;
                      ...
                      return <TableRow key={risk.id}>
                            ...
                            <TableCell>
                              <div className="flex gap-2">
                                ...
                                <Button variant="outline" size="sm" onClick={() => handleEditRisk(risk)}>
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="space-y-2">
                          {governanceRisks.map(risk => <Button key={risk.id} variant={selectedRisk === risk.id ? "default" : "outline"} className="w-full justify-start text-left" onClick={() => setSelectedRisk(risk.id)}>
                              <div>
                                <div className="font-medium">{risk.title}</div>
                                  ...
                                  <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 mb-4">
                                      <div>
                                        ...
                                        <div className="space-y-1">
                                          {risk?.controls.map((control, idx) => <Badge key={idx} variant="outline">{control}</Badge>)}
                                        </div>
                                      </div>
                                      <div>
                                   ...
                                   {plan && <div>
                                       ...
                                       <div className="space-y-3">
                                         {plan.actions.map(action => <Card key={action.id}>
                                             <CardContent className="p-4">
                                       ...
                                       <div className="mt-6 space-y-4">
                                         <div>
                                           ...
                                           <Textarea id="mitigation-comments" placeholder="Adicione comentários, observações ou contexto adicional para enriquecer o plano de mitigação..." value={mitigationComments[selectedRisk] || ""} onChange={e => setMitigationComments(prev => ({
                                    ...prev,
                                    [selectedRisk]: e.target.value
                                  }))} rows={4} className="mt-2" />
                                         </div>
                                         
                                         <Button onClick={() => handleEnhanceWithAI(selectedRisk)} disabled={isEnhancing} className="w-full" variant="outline">
                                           {isEnhancing ? <>
                      ...
                      <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <FormField control={form.control} name="probability" render={({
                            field
                          }) => <FormItem>
                            ...
                            <FormField control={form.control} name="resolutionTime" render={({
                            field
                          }) => <FormItem>
                          ...
                          <FormField control={form.control} name="controls" render={({
                          field
                        }) => <FormItem>
                ...
                <TabsContent value="kpis" className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {kpiData.map((kpi, idx) => <Card key={idx}>
                        <CardContent className="p-4">
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <ResponsiveContainer width="100%" height={300}>
                          <PieChart>
                            <Pie data={RISK_CATEGORIES.map(cat => ({
                            name: cat.name,
                            ...
                            value
                              ...
                              {RISK_CATEGORIES.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                            </Pie>
                            <Tooltip />
                          </PieChart>
                        </ResponsiveContainer>
      ...
      {/* Risk Details Dialog */}
      <RiskDetailsDialog risk={currentRisk} open={detailsDialogOpen} onOpenChange={setDetailsDialogOpen} onEdit={() => currentRisk && handleEditRisk(currentRisk)} />

      {/* Risk Edit Dialog */}
      <RiskEditDialog risk={currentRisk} open={editDialogOpen} onOpenChange={setEditDialogOpen} onSave={handleSaveRisk} />
    </div>;
};
