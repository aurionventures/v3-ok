...
import { BarChart3, TrendingUp, Eye, Calendar, ArrowUpRight, ArrowDownRight, Minus, Clock, User } from "lucide-react";
...
const Maturity = () => {
  ...
  useEffect(() => {
    ...
    const formattedHistory = history.map((assessment, index) => ({
      ...
      overall: assessment.result.pontuacao_total / 20, // Convert to 0-5 scale
      details: convertStoredDataToRadarData(assessment),
    ...
    const logs = history.map((assessment, index) => ({
      id: assessment.id,
      user: assessment.contactInfo?.name || "Usuário",
      userInitials: (assessment.contactInfo?.name || "U").substring(0, 2).toUpperCase(),
            ...
            <CardContent className="p-6">
              ...
              <div className="mt-6 flex justify-end space-x-2">
                ...
                >
                  ...
                  {showActivityLogs ? 'Ocultar Atividades' : 'Exibir Atividades'}
                </Button>
                <Button variant="outline" onClick={goToMaturityQuiz}>
          ...
          <Card className="mb-6">
            <CardContent className="p-6">
              ...
              <div className="bg-white p-4 rounded-lg border mb-6">
                <h3 className="text-lg font-medium mb-4">Evolução da Maturidade</h3>
                <div className="h-80">
                ...
                <div className="mt-4 flex justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm">0-2.9: Baixo</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <div className="h-3 w-3 rounded-full bg-orange-500"></div>
                    <span className="text-sm">3-3.9: Médio</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <div className="h-3 w-3 rounded-full bg-purple-500"></div>
                    <span className="text-sm">4-5: Alto</span>
                  </div>
                </div>
              </div>
            </CardContent>
              ...
              <Table>
                ...
                <TableBody>
                  {historyData.map((assessment, index) => {
                    const maturity = getMaturityLevel(assessment.overall);
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="font-medium">{assessment.overall.toFixed(1)}</TableCell>
                        <TableCell>
                          <Badge className={`${maturity.color} hover:${maturity.color}`}>
                            {maturity.level}
                          </Badge>
